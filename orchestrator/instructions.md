...
- Before saying "sem acesso direto ao feed canonico nesta sessao" for B3, US equities/indices, or FX, call `GetMarketDataSnapshot` for the relevant symbols. If the tool returns current Hostinger MT5 data, use that payload and cite its provenance instead of claiming no data.
